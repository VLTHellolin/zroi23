...
namespace hellolin {
constexpr static ll N = 2e5, LogN = 25;
ll f[N][LogN];
ll find(ll x, ull len) {
    return (x == f[x][len]) ? (x) : (f[x][len] = find(f[x][len], len));
}
void merge(ll x, ll y, ull len) {
    ll fx = find(x, len), fy = find(y, len);
    if(fx!=fy) f[fx][len]=fy;
}
ll n, m;
ll ans;
ll l1, l2, r1, r2;
void solve() {
    ...
    for(ll t=LogN-1; t; t--) {
        ll tt = 1<<t;
        for(ll i=1; i+tt-1<=n; i++) {
            ll ttt = find(i, t);
            merge(ttt, i, t - 1);
            // merge(ttt + tt - 1, i + tt - 1, t - 1);
            merge(ttt + (1<<(t-1)), i + (1<<(t-1)), t - 1);
        }
    }
    for(ll i=1; i<=n; i++) {
        if(i == find(i,0)) {
