...
// * CORE CODE BEGIN * //
bool checkPa(std::string s) {
    std::string ss = s;
    std::reverse(allof(ss));
    return ss == s;
}
bool hasAnswer(std::string s) {
    int tp = 0;
    for(char i:s) {
        tp += i != *s.begin();
    }
...
bool checkIf1Cut(std::string s) {
    std::string ss = s;
    rep(int, i, 1, s.length()) {
        ss = *ss.rbegin() + ss;
        ss.pop_back();
        if(checkPa(ss) && ss!=s) return 1;
    }
    return 0;
}
std::string s;
