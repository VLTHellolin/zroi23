...
constexpr static int N = 5e3, M = N;
...
int n, m, dis[N];
bool bford(int w) {
    std::fill(dis+1, dis+1+n, inf / 2 - 1145);
    dis[w] = 0;
    rep(int, j, 1, n) {
        bool f = 0;
        rep(int, i, 1, m) {
            std::cerr << j << ' ' << i << '\n';
            if(chmin(dis[g[i].v], dis[g[i].u] + g[i].w)) f=1;
        }
        if(!f) return 0;
    }
    return 1;
...
void solve() {
    ...
    if(bford(1)) {
        rep(int, i, 1, n) std::cout << dis[i] << " \n"[i==n];
    } else std::cout << yesno(0, 2);
}
// * CORE CODE END * //
